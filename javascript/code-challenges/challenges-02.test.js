...
const addQuestion = (arr) => {
  // Solution code here...
  let questionMark = arr.map((letter) => {
    let combine = letter + '?';
    return combine;
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    let powerOf = Math.pow(2, arr[i]);
    newArry.push(powerOf);
